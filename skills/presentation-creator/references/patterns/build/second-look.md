...
**Extraction caveat first, because it inverts the result.** This pattern is typically executed with text rendered *inside* images rather than in text boxes. Structural PPTX inspection alone will report these slides as "a single full-bleed image with no text" and score them as `vacation-photos` or `cave-painting` — the exact opposite of what is on screen. After #129 the extractor OCRs picture blobs on low-confidence slides. For cites (fine labels, buried jokes) and the "transcript never mentions the fine labels" check, use only each `ocr_receipts[].recovered_text` whose same receipt has `trustworthy_text: true`; `ocr_text` and channel aggregates preserve untrustworthy review text and are not citational authority. Two-layer **legibility** structure still requires **rendered slide images**; OCR is not a size hierarchy. A deck with near-zero text shapes, large image fills, and dense trustworthy receipt text is a strong candidate, not an image-only deck.

Given rendered images (and trustworthy per-receipt OCR text where present), look for:
