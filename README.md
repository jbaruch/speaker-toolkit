...
- **shownotes-publisher** creates a downloadable Agent Skill from each talk's teaching and writes talk pages into a Jekyll-based shownotes site (e.g., `speaking.jbaru.ch`). Encodes the custom parser's format contract so authored content actually renders: abstract is one paragraph, video field absent = "coming soon" badge, slides/video URLs must be markdown links, no frontmatter title, etc. Uses notes, demos, and available transcripts to teach an audience agent how to apply the talk. Publishes `_skills/{talk-page-stem}/SKILL.md` with the page through a PR and verifies the download. Runs before or after delivery; video-only updates preserve existing skills.
